...
var limitPerPage = 45; // Limit of items per each page
...
$(".pagination li.current-page").on("click", function() {
  ...
  if ($(this).hasClass('active')) {
    ...
    $("#page .textOverImage").hide(); // Hide all items in loop, this case, all the list groups
    var grandTotal = limitPerPage * currentPage; // Get the total number of items up to the page number that was clicked on

    // Loop through total items, selecting a new set of items based on page number
    for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
      $("#page .textOverImage:eq(" + i + ")").show(); // Show items from the new page that was selected
    }
  }
...
$("#next-page").on("click", function() {
  ...
  if (currentPage === totalPages) {
    ...
    $("#page .textOverImage").hide(); // Hide all items in the pagination loop
